...
| 系统架构 | std::shared_ptr<T> 大小 | 组成说明 |
|---------|------------------------|----------|
| 32 位系统 | 8 字节 | 对象指针 (4B) + 控制块指针 (4B) |
| 64 位系统 | 16 字节 | 对象指针 (8B) + 控制块指针 (8B) |
...
| 控制块 | 共享 | 无 |
| 所有权 | 共享所有权 | 独占所有权 |
| 复制行为 | 允许拷贝（引用计数+1） | 禁止拷贝，只能移动 |
| 线程安全 | 引用计数原子操作 | 非线程安全 |
| 内存开销 | 控制块（约16-32字节） | 无额外开销（仅1指针） |
...
class shared_ptr {
    T* ptr;           // 指向实际对象的指针
    ControlBlock* cb; // 指向控制块的指针
};

struct ControlBlock {
    size_t ref_count;     // 强引用计数
    size_t weak_count;    // 弱引用计数
    Deleter* deleter;     // 删除器（可能包含额外信息）
    // 其他元数据...
};
```
代码验证[shared_ptr.cpp](../c++/test/shared_ptr.cpp)
### STL operation
...
pthread_create（基于POSIX线程库）和std::thread（C++标准库）是两种常用的多线程编程接口，它们在平台兼容性、语法特性和资源管理等方面存在显著差异。以下是详细对比：
| 特性       | pthread_create                            | std::thread                                 |
|------------|-------------------------------------------|---------------------------------------------|
| 平台支持   | POSIX系统（如Linux、macOS）                | 所有C++11+标准支持的平台（跨平台，包括Windows、Linux、macOS等） |
| 依赖库     | 需包含<pthread.h>头文件和链接-lpthread库   | C++标准库，无需额外依赖                      |

c++11中多线程对引入了5个头文件atomic,thread,mutex,condition_variable,future.


[example](https://blog.csdn.net/weixin_43971373/article/details/119678930),该例子算是“c++11 并发指南系列里的概述”
...
[一次性搞定右值，右值引用（&&），和move语义](https://juejin.cn/post/6844903497075294216) [对应代码](../c++/test/juejin_reference.cpp)


[一文读懂C++右值引用和std::move](https://zhuanlan.zhihu.com/p/335994370)：const左指可以指向右值。
...
##### atan 
反tan函数，参数是一个浮点型 [cplusplus](https://cplusplus.com/reference/cmath/atan/)
...
##### atan2 
反tan函数扩展，参数是两个浮点型。这样可以求出角度所处的象限。[cplusplus](https://cplusplus.com/reference/cmath/atan2/)
